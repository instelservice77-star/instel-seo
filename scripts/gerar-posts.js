...
Substitua mentalmente {{SERVICO}} por "${servico.nome}" e {{LOCALIDADE}} por "${localidade.nome}" ao escrever o FAQ e os títulos. Siga rigorosamente o formato JSON e as regras definidas nas instruções de sistema.`;
}
...
function montarGaleria(listaImagens, { servico, localidade }) {
  ...
  const itens = listaImagens
    .map((img, i) => {
      const alt = escapeHtml(
        img.alt || `${servico.nome} em ${localidade.nome} — foto ${i + 1}`
      );
      return `<a href="${escapeHtml(img.url)}" target="_blank" rel="noopener" style="flex:1 1 18%;min-width:120px;max-width:200px;">
        <img src="${escapeHtml(img.url)}" alt="${alt}" loading="lazy" style="width:100%;height:140px;object-fit:cover;border-radius:6px;display:block;" />
      </a>`;
    })
    .join("\n");

  return `<div class="instel-galeria" style="display:flex;flex-wrap:wrap;gap:10px;margin:20px 0;">
${itens}
</div>
<style>
@media (max-width: 600px) {
  .instel-galeria a { flex: 1 1 45% !important; max-width: 45% !important; }
}
</style>`;
}
